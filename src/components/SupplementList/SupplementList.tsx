  ...
  return (
    <div className="px-5 py-6">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-lg font-semibold text-slate-100">{t('list.manageSupplements')}</h2>
        <button
          ...
          {state.supplements.map(s => (
            ...
            >
                ...
                <p className="text-xs text-slate-500">
                  {s.frequency.everyNDays === 1
                    ? s.frequency.timesPerDay === 1
                      ? t('card.daily')
                      : t('card.timesDaily', { count: s.frequency.timesPerDay })
                    : t('card.everyNDays', { count: s.frequency.everyNDays })}
                </p>
              </div>
              ...
              <button
                ...
                aria-label={t('list.edit', { name: s.name })}
              >
              ...
              <button
                ...
                aria-label={t('list.delete', { name: s.name })}
              >
                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <ConfirmDialog
        ...
        message={t('list.deleteMessage', { name: deleteTarget?.name || '' })}
